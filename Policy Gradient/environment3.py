...
class Env(tk.Tk):
    def __init__(self):
        ...
        self.goal = []
        # obstacle
        self.set_reward([0, 1], - obstacle_reward)
        self.set_reward([1, np.round(HEIGHT/2)-1], - obstacle_reward)
        self.set_reward([2, HEIGHT-3], - obstacle_reward)
        # #goal
        self.set_reward([WIDTH-1, HEIGHT-1], goal_reward)
        self.set_reward([0, HEIGHT-1], goal_reward)
        self.set_reward([WIDTH-1, 0], goal_reward)
    ...
    def _build_canvas(self):
        ...
        agent3 = canvas.create_image(1.5*UNIT, (HEIGHT-1)*UNIT - UNIT / 2, image=self.shapes[0])
    ...
    def reset(self, agentNr):
        ...
        if agentNr == 0:
            ...
            self.canvas.move(self.agents[agentNr], 1.5*UNIT - x, (HEIGHT-1)*UNIT - UNIT/2 - y)
        self.reset_reward()
    ...
    def step(self, action, agentNr):
        ...
        self.canvas.tag_raise(self.agents[2])
